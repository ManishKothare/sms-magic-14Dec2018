...
if (!function_exists('smsmagic_login_form')){
	function smsmagic_login_form($atts){
		...
		//possible values for intent argument are: webcheckout | webrecharge | resourceaccess | assetLibrary
...
if( !function_exists('salesforce_oauth_response') ){
	function salesforce_oauth_response(){
		...
		$state=urlencode('orgType=production&provider=salesforce_production&redirectUrl=landing_page&redirectionType=signup');
			...
			if( isset($_GET['action']) && $_GET['action']=='sfor' ){
				...
				if( !empty($access_token) && strlen($access_token)>0 ){
					...
					if( $intent=='resourceaccess' ){						
						...
						payee_customer_checks($user_profile_info, $redirect_page);
				...
				if( isset($access_token) && strlen($access_token)>0 ){ //set login successful
					...
					if( isset($login_intent) && ($login_intent!=='' || strlen($login_intent)>0) ){
						...
						if($login_intent=='resourceaccess'){
							...
							//Check for payee customer with salesforce version
							payee_customer_checks($user_profile_info, $referer);
						...
						if($login_intent=='assetLibrary'){
							//call profile info checking function
							//user_profile_checks($user_profile_info);
							//$asset_page='/asset-library/';
							//wp_safe_redirect( $asset_page );
							//wp_safe_redirect( get_site_url().'/asset-library/' );
							//exit;
							asset_info_checks($user_profile_info, $referer);
						}
						//for web checkout ends
					}else{
...
if(!function_exists('asset_info_checks')){
	function asset_info_checks($user_profile, $referer){	        
	        $names=$user_profile['name'];
	        $industry=$user_profile['industry'];
	        $industry_name=$user_profile['industry_name'];
	        		
		session_start();
		$_SESSION['FullName'] = $names;
		$_SESSION['IndustryName'] = $industry_name;
		$_SESSION['LoggedIn']='Manish';		
...
/*
this is logic to get the account type details
if account_type_id == 4 and user_type_id == 4: account_type = 'salesforce'
elif account_type_id == 2 and source == 80000181: account_type = 'zoho'
elif account_type_id == 4 and user_type_id ==1: account_type = 'email'
*/

if(!function_exists('payee_customer_checks')){
	function payee_customer_checks($user_profile, $referer){		
		...
		if( $account_type == 4 && $user_type == 4 ){ //check for Salesforce Users
			wp_safe_redirect( get_site_url().$referer );
			exit;				
		}
		if( $account_type == 2 && $source == 80000181 ){ //Check for Zoho users
			wp_safe_redirect( get_site_url().'/technical-resource-center/sms-magic-guide-for-zoho-users/' );
			exit;			
		}
		if( $account_type == 4 && $user_type == 1 ){ //Check for Email users
			wp_safe_redirect( get_site_url().'/technical-resource-center/for-other-users/' );
			exit;			
		}
	}
}
...
if( !function_exists('techdocs_access_check') ){
	function techdocs_access_check($page){				
		...
		if( (isset($_SESSION['LoggedIn'])) && ($_SESSION['LoggedIn']==TRUE) ){ //loggedin check
			if( $account_type == 4 && $user_type == 4 ){ //check for Salesforce Users
				if( strpos($page,'sms-magic-guide-for-zoho-users')!==FALSE ){
					wp_safe_redirect( get_site_url().'/technical-resource-center/for-salesforce-users/' );
					exit;
				}
			}
			if( $account_type == 2 && $source == 80000181 ){ //check for Zoho Users
				if( (strpos($page,'sms-magic-guide-for-salesforce-admin')!==FALSE) || (strpos($page,'sms-magic-guide-for-salesforce-users')!==FALSE) ){
					wp_safe_redirect( get_site_url().'/technical-resource-center/for-zoho-users/' );
				        exit;
				}
			}
			if( $account_type == 4 && $user_type == 1 ){ //check for Email Users
				if( (strpos($page,'sms-magic-guide-for-salesforce-admin')!==FALSE) || (strpos($page,'sms-magic-guide-for-salesforce-users')!==FALSE) || (strpos($page,'sms-magic-guide-for-zoho-users')!==FALSE) ){
					wp_safe_redirect( get_site_url().'/technical-resource-center/for-other-users/' );
			                exit;
				}
			}
		}else{
			if( (strpos($page,'sms-magic-converse-guide-for-salesforce-admin')!==FALSE) || (strpos($page,'sms-magic-interact-guide-for-salesforce-admin')!==FALSE) ){
				wp_safe_redirect( get_site_url().'/technical-resource-center/sms-magic-guide-for-salesforce-admin/' );
			        exit;
			}
			if( (strpos($page,'sms-magic-converse-guide-for-salesforce-users')!==FALSE) || (strpos($page,'sms-magic-interact-guide-for-salesforce-users')!==FALSE) ){
				wp_safe_redirect( get_site_url().'/technical-resource-center/sms-magic-guide-for-salesforce-users/' );
			        exit;
			}		
		}
	}
	...
	function smsmagic_sfdc_oauth_trialorg(){
		...
		if( isset($_GET) ){//check for the GET values exist
			...
			if( !empty($access_token) && strlen($access_token)>0 ){
				//get profile info via api call
				$user_profile_info=get_user_profile_info($access_token);
				//https://launch.sms-magic.com/appexchange-converse-trial/select-option/
