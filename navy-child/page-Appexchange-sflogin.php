<?php
/**
 * Template Name: 	Appexchange SF Login Page
 * Navy Theme:		Appexchange SF Login Page
 * @package:		WordPress
 ...
 */
	...
	get_header('appexchange-sflogin');
 ?>
 ...
 <div id="<?php echo esc_html($post->post_name);?>" class="row standAlonePage <?php echo esc_attr($hgr_page_color_scheme);?>" style=" <?php echo esc_attr($backgroundColor);?> ">
  ...
  <?php endif;?>
    ...
    <div class="container" style=" <?php echo ( !empty($hgr_page_top_padding) ? ' padding-top:'.esc_attr($hgr_page_top_padding).'px!important;' : '' ); echo ( !empty($hgr_page_btm_padding) ? ' padding-bottom:'.esc_attr($hgr_page_btm_padding).'px!important;' : '' );?> ">
      ...
      <!--<div class="SMS-Magic_Logo"><a href="<?php //echo get_site_url()?>"><img src="<?php //echo get_site_url()?>/wp-content/uploads/logos/logo.png" alt="logo" /></a></div>-->
      <div style="margin-top:-34px;">
        <?php the_content(); ?>
        
        <?php echo do_shortcode('[smsmagic_signin_with_sfdc buttonid="sfbtn"]');?>
...
(function($){
    $(document).ready(function(){
        $("#sfbtn").attr("disabled", "disabled");
        $("#sfbtn").addClass("dissable-btn");	   
	$("#consentStatus").click(function() {
	    var checked_status = this.checked;
	    if (checked_status == true) {
	      $("#sfbtn").removeAttr("disabled");
	      $("#sfbtn").removeClass("dissable-btn");
	    } else {
	      $("#sfbtn").attr("disabled", "disabled");
	      $("#sfbtn").addClass("dissable-btn");
	    }
